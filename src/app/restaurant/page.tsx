...
export default function RestaurantDashboardPage({ onNavigate }: RestaurantDashboardPageProps) {
    ...
    return (
        <div className="flex flex-col h-screen bg-muted/10 relative overflow-hidden">
            ...
            <header className="p-4 md:p-6 bg-white border-b shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 shrink-0 z-20">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                     <Button 
                        variant="outline" 
                        className={cn(
                            "font-black border-2 rounded-xl transition-all shadow-sm h-12 flex-1 md:flex-none",
                                    ...
                                    <CardContent className="p-4 pt-0">
                                        <ul className="space-y-1">
                                            {order.items.map(item => (
                                                <li key={item.product.id + (item.selectedSize?.name || '')} className="flex justify-between text-sm flex-row-reverse">
                                    ...
                                    <CardHeader className="p-4">
                                     ...
                                     <CardContent className="p-4 pt-0">
                                        {order.status === 'confirmed' && order.deliveryWorker ? (
                                            <div className="flex items-center gap-2 p-2 bg-green-50 rounded-xl text-green-700 border border-green-100 mb-3 flex-row-reverse">
                                         ...
                                         <ul className="space-y-1 opacity-80">
                                            {order.items.map(item => (
                                                <li key={item.product.id + (item.selectedSize?.name || '')} className="text-xs font-bold flex justify-between flex-row-reverse">
            ...
            <Dialog open={!!newOrderAlert} onOpenChange={() => {}}>
                <DialogContent className="sm:max-w-md bg-white rounded-t-[3rem] border-none shadow-2xl p-0 overflow-hidden">
                    ...
                    <div className="p-6 space-y-4 text-right">
                        <div className="flex justify-between items-center text-xl font-black text-primary border-b-2 border-dashed pb-4 flex-row-reverse">
                            <span className="flex items-center gap-2 flex-row-reverse"><ReceiptText className="h-5 w-5"/> رقم الطلب:</span>
                            ...
                            <ScrollArea className="max-h-[35vh] rounded-2xl border-2 border-muted bg-muted/20 p-2">
                                <ul className="space-y-3">
                                    {newOrderAlert?.items.map((item, idx) => {
                                        ...
                                        return (
                                            <li key={idx} className="flex justify-between items-center p-3 bg-white rounded-xl shadow-sm flex-row-reverse">
                                                <div className="flex flex-col text-right">
                                                    <span className="font-black text-base">{item.quantity}x {item.product.name}</span>
                                                    {item.selectedSize && <Badge variant="secondary" className="w-fit text-[9px] mt-1 mr-auto">{item.selectedSize.name}</Badge>}
                                                </div>
