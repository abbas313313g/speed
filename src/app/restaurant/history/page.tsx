...
export default function RestaurantHistoryPage({ onBack }: RestaurantHistoryPageProps) {
    ...
    const handleWithdraw = async () => {
        if (!restaurant || totalIncome < 5000) {
            toast({ title: "الرصيد المتاح غير كافٍ", description: "الحد الأدنى لطلب السحب هو 5,000 د.ع", variant: "destructive" });
            return;
        }
    ...
    return (
        <div className="p-4 space-y-6 bg-slate-50 h-full overflow-y-auto pb-32 text-right">
            ...
            <div className="space-y-4">
                <h2 className="text-lg font-black flex items-center gap-2 px-1 justify-end text-slate-800">سجل عمليات السحب <History className="h-5 w-5 text-primary"/></h2>
