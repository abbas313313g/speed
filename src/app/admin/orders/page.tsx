...
import { MoreHorizontal, Trash2, Loader2, Search, CheckSquare, Phone, Store, User, MapPin, Receipt, X, UserCog, RefreshCw } from 'lucide-react';
...
export default function AdminOrdersPage({ branchId }: { branchId: string }) {
  ...
  const handleReassign = async (orderId: string) => {
      try {
          ...
          toast({ title: "تم سحب الطلب وإعادته لدوامة البحث ♻️" });
      } catch(e) {}
  };
    ...
    <div className="space-y-8 text-right">
      <header className="flex justify-between items-start">
        <div>
            ...
            <p className="text-muted-foreground font-bold">متابعة دقيقة وتحكم كامل في مهام المناديب والمتاجر.</p>
        </div>
            ...
            <AlertDialog>
                ...
                <AlertDialogContent className="rounded-[2.5rem]">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-right font-bold text-muted-foreground">أنت على وشك حذف {selectedOrderIds.length} طلب نهائياً. لا يمكن التراجع.</AlertDialogDescription>
                    </AlertDialogHeader>
        ...
        {allOrders.length > 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] border dark:border-slate-800 shadow-xl overflow-hidden">
            <Table>
                ...
                {allOrders.map((order) => (
                    ...
                    <TableCell>
                        ...
                        ) : <span className="text-[10px] text-muted-foreground italic">لم يعين</span>}
                    </TableCell>
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                        <div className="flex justify-center items-center gap-1">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuItem onClick={() => handleUpdateStatus(order.id, 'pending_assignment')} className="gap-2"><RefreshCw className="h-4 w-4"/> إعادة للبحث</DropdownMenuItem>
                                ...
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => handleReassign(order.id)} className="text-orange-600 gap-2"><UserCog className="h-4 w-4"/> تغيير المندوب</DropdownMenuItem>
                                <DropdownMenuSeparator />
                                <AlertDialogTrigger asChild>
                            ...
                            <AlertDialogContent className="rounded-[2.5rem]">
                                <AlertDialogHeader><AlertDialogTitle className="text-right">تأكيد الحذف</AlertDialogTitle><AlertDialogDescription className="text-right">سيتم مسح سجل الطلب تماماً من النظام والحسابات.</AlertDialogDescription></AlertDialogHeader>
            ...
            <div className="p-20 text-center bg-white dark:bg-slate-900 rounded-[2rem] border-2 border-dashed">
                    ...
                    <div className="flex flex-col text-right">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="space-y-4">
                                <h3 className="font-black flex items-center gap-2 justify-end"><Receipt className="h-5 w-5 text-primary"/> الأصناف المطلوبة</h3>
                                <div className="space-y-2">
                                    {viewOrder.items.map((item, idx) => (
                                        <div key={idx} className="flex justify-between items-center p-3 bg-muted/20 rounded-xl">
                                            <div className="font-black text-primary">x{item.quantity}</div>
                            ...
                            <div className="space-y-2">
                                <h3 className="text-[10px] font-black text-muted-foreground uppercase flex items-center gap-1 justify-end"><MapPin className="h-3 w-3"/> الوجهة</h3>
                                <div className="p-4 bg-muted/20 rounded-xl text-xs font-bold">
                                    <p className="text-primary font-black mb-1">{viewOrder.address.deliveryZone}</p>
                                    <p>{viewOrder.address.details || 'لا توجد ملاحظات.'}</p>
                                </div>
                            </div>
                        </div>

                        <DialogFooter className="p-6 bg-slate-50 border-t sticky bottom-0">
                            <Button onClick={() => setViewOrder(null)} className="w-full h-14 rounded-2xl font-black text-lg">إغلاق</Button>
                        </DialogFooter>
                    </div>
                )}
            </DialogContent>
