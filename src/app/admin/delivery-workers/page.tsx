...
import { Wallet, Banknote, UserCheck, Loader2, CheckCircle2, RotateCcw, Printer, Clock, ChevronDown, ChevronUp, ReceiptText } from 'lucide-react';
      ...
      const htmlContent = `
          ...
          <head>
              ...
              <style>
                  ...
                  .order-table th, .order-table td { border: 1px solid #eee; padding: 8px; text-align: right; font-size: 12px; }
          ...
          <body>
              ...
              <div class="section-box profit-box">
                  <p style="font-size: 14px; font-weight: bold;">أرباح التوصيل المستحقة: ${formatCurrency(worker.totalWallet)}</p>
                  <table class="order-table">
                      <thead><tr><th>رقم الطلب</th><th>التاريخ</th><th>الأجرة</th></tr></thead>
                      <tbody>
                          ${worker.unpaidEarningsOrders.map((o: any) => `<tr><td>#${o.orderNumber}</td><td>${new Date(o.date).toLocaleDateString('ar-IQ')}</td><td>${formatCurrency(o.deliveryFee)}</td></tr>`).join('')}
                      </tbody>
                  </table>
              </div>

              <div class="section-box debt-box">
                  <p style="font-size: 14px; font-weight: bold;">ذمة الكاش (للمكتب): ${formatCurrency(worker.totalDebt)}</p>
                  <table class="order-table">
                      <thead><tr><th>رقم الطلب</th><th>التاريخ</th><th>مبلغ الكاش</th></tr></thead>
                      <tbody>
                          ${worker.unpaidDebtOrders.map((o: any) => `<tr><td>#${o.orderNumber}</td><td>${new Date(o.date).toLocaleDateString('ar-IQ')}</td><td>${formatCurrency(o.total)}</td></tr>`).join('')}
                      </tbody>
                  </table>
              </div>
          ...
          <div className="grid gap-6 pb-20">
              {workerBalances.map((w) => (
                  <Card key={w.id} className="rounded-[2.5rem] border-none shadow-xl overflow-hidden bg-white">
                      <div className="bg-primary/5 p-6 border-b border-dashed flex justify-between items-center flex-row-reverse cursor-pointer" onClick={() => setExpandedWorkerId(expandedWorkerId === w.id ? null : w.id)}>
                          <div className="text-right">
                              <div className="flex items-center gap-2 justify-end">
                                  <h3 className="text-xl font-black">{w.name}</h3>
                                  <Button variant="outline" size="icon" className="rounded-xl h-9 w-9 border-2" onClick={(e) => { e.stopPropagation(); handlePrintWorkerReport(w); }}>
                                      <Printer className="h-4 w-4 text-primary" />
                                  </Button>
                                  {expandedWorkerId === w.id ? <ChevronUp className="h-4 w-4 text-muted-foreground ml-2"/> : <ChevronDown className="h-4 w-4 text-muted-foreground ml-2"/>}
                              </div>
                      ...
                      {expandedWorkerId === w.id && (
                          <div className="p-6 bg-slate-50 border-t border-dashed animate-in slide-in-from-top-2">
                              <div className="grid md:grid-cols-2 gap-8">
                                  <div>
                                      <div className="flex items-center gap-2 mb-3 justify-end text-primary font-black text-xs">
                                          <span>طلبات الأرباح المعلقة</span>
                                          <ReceiptText className="h-3 w-3"/>
                                      </div>
                                      <div className="space-y-2">
                                          {w.unpaidEarningsOrders.map(o => (
                                              <div key={o.id} className="flex justify-between p-2 bg-white rounded-lg border text-[10px] font-bold">
                                                  <span className="text-primary">{formatCurrency(o.deliveryFee)}</span>
                                  ...
                                  <div>
                                      <div className="flex items-center gap-2 mb-3 justify-end text-destructive font-black text-xs">
                                          <span>فواتير الكاش المعلقة</span>
                                          <Banknote className="h-3 w-3"/>
                                      </div>
                                      <div className="space-y-2">
                                          {w.unpaidDebtOrders.map(o => (
                                              <div key={o.id} className="flex justify-between p-2 bg-white rounded-lg border text-[10px] font-bold">
                                                  <span className="text-destructive">{formatCurrency(o.total)}</span>
                                                  <span>#{o.orderNumber}</span>
                                              </div>
                                          ))}
                                      </div>
