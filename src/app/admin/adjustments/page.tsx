            ...
            <div className="grid md:grid-cols-2 gap-8">
                <Card className="rounded-[2.5rem] border-none shadow-2xl bg-white p-8">
                    <CardHeader className="p-0 mb-8">
                        <CardTitle className="text-2xl font-black flex items-center gap-2 justify-end">إجراء خصم جديد <ShieldAlert className="text-destructive h-6 w-6"/></CardTitle>
                    </CardHeader>
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <Label className="font-black text-sm">نوع الجهة</Label>
                        ...
                        <div className="space-y-2">
                            <Label className="font-black text-sm">اختيار المندوب/المتجر</Label>
                        ...
                        {type === 'delivery' && (
                            <div className="space-y-3">
                                <Label className="font-black text-sm">المحفظة المستهدفة</Label>
                                <div className="grid grid-cols-2 gap-3">
                                    <button 
                                        onClick={() => setAdjustmentField('balanceAdjustment')}
                                        className={`p-4 rounded-2xl border-2 text-right transition-all ${adjustmentField === 'balanceAdjustment' ? 'border-primary bg-primary/5' : 'border-muted'}`}
                                    >
                                        <Wallet className="h-5 w-5 text-primary mb-2"/>
                                        <p className="font-black text-xs">أرباح المندوب</p>
                                        <p className="text-[9px] font-bold text-muted-foreground">الخصم من رصيده المستحق</p>
                                    </button>
                                    <button 
                                        onClick={() => setAdjustmentField('debtAdjustment')}
                                        className={`p-4 rounded-2xl border-2 text-right transition-all ${adjustmentField === 'debtAdjustment' ? 'border-destructive bg-destructive/5' : 'border-muted'}`}
                                    >
                                        <Banknote className="h-5 w-5 text-destructive mb-2"/>
                                        <p className="font-black text-xs">ذمة المكتب</p>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input 
                                ...
                                placeholder="0" 
                                className="h-16 rounded-2xl text-3xl font-black text-center text-destructive bg-muted/20 border-none"
                            />
                        </div>

                        <div className="space-y-2">
                            <Label className="font-black text-sm">سبب الخصم (اختياري)</Label>
                            <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="مثال: غرامة تأخير، تعويض زبون..." className="h-12 rounded-xl" />
                        </div>
                ...
                <div className="space-y-6">
                    <div className="p-8 bg-primary/5 rounded-[3rem] border-4 border-dashed border-primary/20 space-y-4">
                        <div className="p-4 bg-white rounded-3xl w-fit"><Landmark className="h-10 w-10 text-primary"/></div>
                        <h2 className="text-2xl font-black">ملاحظات المحاسبة</h2>
                        <ul className="space-y-3 font-bold text-sm text-slate-600 list-disc list-inside pr-2">
